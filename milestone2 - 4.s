...
.equ PERIOD1, 75000 # time period for turning motor on (NOT INTERRUPT MODE)
.equ PERIOD2, 150000 # time period for turning motor off (NOT INTERRUPT MODE)
.equ PERIOD3, 10000000 # time period for turning motor on (INTERRUPT MODE)
.equ PERIOD4, 400000000 # time period for motor to stop after moving backward (INTERRUPT MODE)
...
STRAIGHT_TURN:
		...
		# r8 used to decide which direction to go
		movi r8, 0
...
MOTOR_TURN_LEFT:
		...
		movui r6, %lo(PERIOD3)
		movui r7, %hi(PERIOD3)
        call StartTimer1 # polling for PERIOD3 amount of time
...
MOTOR_MOVE_BACKWARD:
		...
		# r8 is 1 then lego car should turn left
		bne r8, r0, MOTOR_TURN_LEFT
        br MOTOR_TURN_RIGHT
